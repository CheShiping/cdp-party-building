...
<template>
  ...
  >
    ...
    <view
      class="app-navbar__capsule"
      :style="{ height: `${capsuleBandHeight}px` }"
    >
      <view
        v-if="showBack"
        ...
        @tap="handleBack"
      >
        <text class="app-navbar__back-icon">
          ←
        </text>
      </view>
    </view>
    ...
    <!-- 标题行：位于胶囊带下方，与胶囊不重叠 -->
    <view
      class="app-navbar__title-row"
      :style="{ height: `${titleRowHeight}px` }"
    >
      <text class="app-navbar__title">
        {{ title }}
      </text>
    </view>
  </view>
